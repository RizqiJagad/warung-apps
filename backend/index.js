...
const main = async () => {
  try {
    ...
    app.get('/api/barang', async (req, res) => {
      try {
        ...
        const items = rows.map((row, index) => ({
          id: row.get('id') || `row-${index + 1}`, // fallback id
    ...
    // --- API ENDPOINT UNTUK MELIHAT SEMUA TRANSAKSI ---
        app.get('/api/transaksi-list', async (req, res) => {
            try {
                const rows = await transaksiSheet.getRows();
                
                // Urutkan dari transaksi terbaru
                const latestTransactions = rows.map(row => ({
                    tanggal: row.get('tanggal'),
                    nama_barang: row.get('nama_barang'),
                    jumlah_terjual: row.get('jumlah_terjual'),
                    total_harga: row.get('total_harga')
                })).reverse();

                res.status(200).json({ success: true, data: latestTransactions });
            } catch (error) {
                console.error('Error saat mendapatkan daftar transaksi:', error);
                res.status(500).json({ success: false, message: 'Gagal mendapatkan daftar transaksi.', error: error.message });
            }
        });
    ...
    app.put('/api/barang/:id', async (req, res) => {
      try {
        ...
        let rowToEdit = rows.find(row => row.get('id') === id);
        if (!rowToEdit && id.startsWith('row-')) {
          rowToEdit = rows[parseInt(id.split('-')[1], 10) - 1];
        }
    ...
    app.delete('/api/barang/:id', async (req, res) => {
      try {
        ...
        let rowToDelete = rows.find(row => row.get('id') === id);
        if (!rowToDelete && id.startsWith('row-')) {
          rowToDelete = rows[parseInt(id.split('-')[1], 10) - 1];
        }
    ...
    // --- Laporan Laba Harian, Mingguan, Bulanan ---
